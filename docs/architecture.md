...
|  +--------+--------+--------+--------+--------+--------+--------+ |
<!-- FIXED (v4.3.0): DOC-ARCH-PATH -->
|  |console |keyboard| irq.c  |exception.c| pit.c  |pagetable.c|capability.c| |
|  +--------+--------+--------+--------+--------+--------+--------+ |
|                                                                   |
|  +--------+--------+--------+--------+--------+--------+--------+ |
...
|------|------|------|
<!-- FIXED (v4.3.0): DOC-ARCH-PATH -->
| KASLR 实现 | `aslr.c` | 内核加载地址随机化、ChaCha20 CSPRNG 熵源、重定位处理 |
| KASLR 头文件 | `aslr.h` | KASLR 偏移量、随机化接口定义 |
...
|------|------|------|
<!-- FIXED (v4.3.0): DOC-ARCH-PATH -->
| AF_UNIX 实现 | `net/unix.c` | 本地进程间通信、流式/数据报套接字、抽象命名空间 |
| AF_UNIX 头文件 | `net/unix.h` | Unix 域地址结构体、协议常量定义 |
...
| ld.so 实现 | `ldso.c` | [计划中] 用户态 ELF 动态链接、共享库加载、符号解析与重定位 |
| ld.so 头文件 | `ldso.h` | [计划中] 动态链接器接口、辅助向量增强定义 |
