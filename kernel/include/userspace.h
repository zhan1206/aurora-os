 ...
 *
 * Without SMAP: validates address range is in user space.
 * With SMAP: uses STAC/CLAC to temporarily allow kernel access to user pages.
 *
 * Returns 0 on success, -EFAULT if any byte is in kernel space.
 */
#ifndef USERSPACE_H
#define USERSPACE_H
...
/* Forward declarations from string.h (needed for freestanding builds) */
#include <string.h>
 ...
 * Returns 0 on success, -EFAULT if src range is invalid.
 */
 ...
 * @src:  kernel buffer.
 * @size: number of bytes to copy.
 * Returns 0 on success, -EFAULT if dst range is invalid.
 */
