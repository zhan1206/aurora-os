...
v4.4.2 修复 v4.4.1 中仍存的 15 处编译/构建缺口，新建 4 个 QA 脚本 + CI 门禁，实现 `make` 0 错 0 警告。

### 编译修复 (15项)

| 类别 | 项数 | 关键修复 |
|------|------|----------|
| 注释嵌套 | 6 | user_access.h, hid.c/h, usb.h, xhci.h, xhci_dma.h 内层 `/* */` 移除 |
| 头文件缺失 | 5 | acpi.h/syscall.h 加 `<stddef.h>`, module.c 加 `<errno.h>`, main.c 加 `vfs.h`, hid_event.c 加 `<string.h>` |
| 未声明符号 | 2 | aslr.c: extern kernel_cr3; drm.c: LOG_LEVEL_ERROR→LOG_LEVEL_ERR |
| 类型冲突 | 2 | sys_fork 签名统一 `int flags`; seccomp_set_filter 1参→2参桩 |
| 重复定义 | 1 | hid_event.c 移除重复 struct hid_event |
| 缺失文件 | 1 | 新建 `kernel/include/syscall_numbers.h` (42行) |
| 结构体缺成员 | 1 | tmpfs_inode 补 `struct inode inode` |
| 宏定义顺序 | 1 | UDP_SOCKET_MAGIC 移到首次使用前 |

### QA脚本 (5个)

| 编号 | 脚本 | 用途 |
|------|------|------|
| CI-007 | `scripts/check_nested_comments.sh` | 扫描 `/* */` 嵌套注释，永阻回归 |
| CI-008 | `scripts/check_header_includes.sh` | 检查 .c 文件是否 include 对应 .h |
| CI-009 | `scripts/check_static_vs_header.sh` | 检测 static 函数与 extern 声明冲突 |
| CI-010 | `scripts/audit_changelog.sh` | 比对 CHANGELOG.md 与 git tag/version.h |
| CI-011 | `.github/workflows/build.yml` | CI 集成: 运行全部 4 个 QA 脚本 |

### 构建增强

| 编号 | 改进 |
|------|------|
| BUILD-12 | Makefile: `-Werror=enum-conversion` 捕获 enum/int 混用 |

### CHANGELOG 诚实化

- v4.4.0: "60+自检"→"60+测试项"，注明部分为基础边界测试
- v4.4.0: -Werror=comment 说明补充"部分嵌套注释仍残留，于 v4.4.2 修复"
...
|------|------|
| 修改文件 | 20+ |
| 新建文件 | 5 (syscall_numbers.h + 4 scripts) |
| 新增代码 | ~500 行 |

### 版本历史

```
v4.4.2 ← 当前 (编译缺口修复: 15项+5脚本, 0错0警告)
v4.4.1 (可重现构建+文档一致+短板验证: 10项)
v4.4.0 (里程碑: 编译零警告, 60+测试项, CI门禁, USB编译修复)
...
v4.4.1 实现路线图 Phase A 剩余目标：可重现构建、文档一致性、自检时效约束、已知短板修复。本次同步修正 CHANGELOG 中 v4.4.0 条目的准确性（部分措辞与实际交付有偏差）。
...
- 修复编译失败（USB 6文件嵌套注释）
- 构建零警告（-Werror=comment 已添加，部分嵌套注释仍残留，于 v4.4.2 修复）
- 自检从26组扩展到60+测试项（含基础边界测试）
- CI门禁（嵌套注释检测+零警告构建）
...
| 文件 | 更新 |
|------|------|
| LIMITATIONS.md | USB注释嵌套→已修复, xhci拼写→已修复, 重定义→已修复, 自检13→60+测试项, BUILD-001新增 |
| README.md | 自检数26→60+测试项, 版本v4.3.9→v4.4.0 |
